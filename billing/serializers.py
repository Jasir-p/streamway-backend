...
from tenant.serializer import TenantViewSerializer

class TenantBillingSerializer(serializers.ModelSerializer):
    tenant = TenantViewSerializer(read_only=True)
    ...
    class Meta:
        model = TenantBilling
        fields = [
            'id','tenant','tenant_name', 'stripe_customer_id', 'billing_email', 
            'billing_name', 'per_user_rate', 'billing_expiry', 'is_active',
            'last_billed_date', 'next_billing_date','active_count_users','bill_amount','last_billing_status'
        ]
...
class InvoiceSerializer(serializers.ModelSerializer):
    ...
    tenant_billing = TenantBillingSerializer(read_only=True)
